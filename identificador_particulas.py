...
# Abre o arquivo TIFF
path_imagem = Path(easygui.fileopenbox("Escolha o arquivo a ser analisado"))
...
for frame_idx in tqdm(range(len(frames)), desc="Analisando frames"):
    imagem_binaria = cv2.threshold(frames[frame_idx], limiar, 255, cv2.THRESH_BINARY)[1].astype(np.uint8)
    ...
    for pos_centro in centroides[1:].astype(int):
        ...
        if centro_tuple in contagem_centros:
            ...
            contagem_centros[centro_tuple] = 1
        cv2.circle(imagem_marcada, (centro_tuple[0], centro_tuple[1]), 3, (0,0,255), 1)
